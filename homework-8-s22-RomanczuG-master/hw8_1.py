...
def get_formatted_text(filename) :
    ...
    with open(filename, "r") as myfile:
        ...
        lines = ["__"+ i.lower() +"__" for i in docs]
    #fill in
...
def get_dict(filename, n):
    ...
    for line in formatd_lines:
        ngram_list.extend(get_ngrams(line,n))

    ngram_dict = dict.fromkeys(ngram_list, 0)

    for k, v in ngram_dict.items():
        ngram_dict[k] = ngram_list.count(k)
...
def top_N_common(filename,N,n):
    ...
    common_N = sorted(get_dict(filename,n).items(), key=lambda x: x[1], reverse=True)[:N]

    for k in list (common_N) [0 : N]:
        common_N.append((k, common_N[ k ]))
...
def dict_union(listOfDicts):
    ...
    dict_set={key for i in listOfDicts for key, value in i.items()}
    union_ngrams.extend(sorted(set(dict_set)))
...
def compare_langs(langFiles,N,n):
    ...
    for langFile1 in langFiles:
        for langFile2 in langFiles:
            if langFile1 != langFile2:
                n_grams1 =  set([tup[0] for tup in top_N_common(langFile1,N,n)])
                n_grams2 =  set([tup[0] for tup in top_N_common(langFile2,N,n)])
                similarity = len(n_grams1.intersection(n_grams2)) / N
                if best_similarity < similarity:
                    best_similarity = similarity
                    most_similar_pair = (langFile1, langFile2, similarity)
